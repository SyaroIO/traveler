...
export const isEmail = (email: string) => isString(email) && emailRule.test(email)
export const isName = (name: string) => isString(name) && nameRule.test(name)

const encryptDocPassword = (doc: { password: string }) => ({ ...doc, password: encryptPassword(doc.password) })
